...
import { BeautifulApiClient } from './beautifulApiClient'
...
export class BaseService {
  ...
  constructor(private serviceBuilder: BeautifulApiClient) {
    this.endpoint = serviceBuilder.endpoint
    this.httpClient = new HttpClientWrapper(serviceBuilder)
  ...
  async __performRequest<T = DataType | DataType[]>(methodName: keyof this, args: unknown[] = []): Promise<HttpApiResponse<T>> {
  ...
  private async _wrapToAxiosResponse<T = DataType | DataType[]>(methodName: string, args: unknown[]): Promise<HttpApiResponse<T>> {
    ...
    const result = await this.httpClient.sendRequest<T>(config)
  ...
  private _responseValidator<T extends unknown | Array<unknown>>(response: AxiosResponse<T>, methodName: string, metadata: MethodMetadata) {
  ...
  private _resolveParameters(
    ...
    if (headers[CONTENT_TYPE_HEADER] && (headers[CONTENT_TYPE_HEADER] as string).indexOf(CONTENT_TYPE.MULTIPART_FORM_DATA) !== -1 && isNode) {
