...
export const GET = <T extends BaseService>(url: string, options?: HttpMethodOptions) => registerMethod<T>('GET', url, options)

export const POST = <T extends BaseService>(url: string, options?: HttpMethodOptions) => registerMethod<T>('POST', url, options)
export const PUT = <T extends BaseService>(url: string, options?: HttpMethodOptions) => registerMethod<T>('PUT', url, options)
export const PATCH = <T extends BaseService>(url: string, options?: HttpMethodOptions) => registerMethod<T>('PATCH', url, options)
export const DELETE = <T extends BaseService>(url: string, options?: HttpMethodOptions) => registerMethod<T>('DELETE', url, options)
export const HEAD = <T extends BaseService>(url: string, options?: HttpMethodOptions) => registerMethod<T>('HEAD', url, options)
export const OPTIONS = <T extends BaseService>(url: string, options?: HttpMethodOptions) => registerMethod<T>('OPTIONS', url, options)
 ...
 */
export const FormUrlEncoded = <T extends BaseService>(target: T, methodName: string, descriptor: PropertyDescriptor) => {
  Headers({ [CONTENT_TYPE_HEADER]: `${CONTENT_TYPE.FORM_URL_ENCODED};${CHARSET_UTF_8}` })(target, methodName, descriptor)
}
 ...
 *         })
 */
export const RequestTransformer = <T extends BaseService, P1 extends DataType>(...transformers: TransformerType<P1>[]) => {
  return (target: T, methodName: string) => {
 ...
 *         })
 */
export const ResponseTransformer = <T extends BaseService, P1 extends DataType | DataType[]>(...transformers: TransformerType<P1>[]) => {
  return (target: T, methodName: string) => {
    target.__getServiceMetadata().setMetadata(methodName, (prev) => ({
      ...prev,
