import { BeautifulApiClient, ValidationErrors } from '../../../src'
...
import { ValidationMethod } from '../../../src/constants'
...
describe('Validate', () => {
  describe('Pass', () => {
    describe('Raw response', () => {
      ...
      beforeAll(() => {
        service = new BeautifulApiClient(testServer.url, { validateResponse: ValidationMethod.CLASS_VALIDATOR }).build(ValidationPassService)
      })
    ...
    describe('Inlined response', () => {
      ...
      beforeAll(() => {
        service = new BeautifulApiClient(testServer.url, {
          validateResponse: ValidationMethod.CLASS_VALIDATOR,
  ...
  describe('Fail', () => {
    describe('Raw response', () => {
      ...
      beforeAll(() => {
        service = new BeautifulApiClient(testServer.url, { validateResponse: ValidationMethod.CLASS_VALIDATOR }).build(ValidationFailService)
      })
      ...
      ])('[%s] - %s', async (msg1: string, msg2: string, fn: (s: ValidationFailServiceInlinedBody | ValidationFailService) => void) => {
        await fn(service)
      })
