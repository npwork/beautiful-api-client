...
describe('Decorators - Multipart', () => {
  test('@Multipart', async () => {
    const fileService = new BeautifulApiClient(testServer.url).build(FileService)
    const bucket = {
      value: 'test-bucket',
    }
    const file = {
  ...
  test('@Multipart - test2', async () => {
    const messagingService = new BeautifulApiClient(testServer.url).build(MessagingService)
