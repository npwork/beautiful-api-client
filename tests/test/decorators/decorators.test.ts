...
describe('Decorators', () => {
  ...
  beforeAll(() => {
    service = new BeautifulApiClient(testServer.url, { log: true }).build(PostsApiService)
  })
  ...
  test('@GET - without base path', async () => {
    const service = new BeautifulApiClient(testServer.url).build(ServiceWithoutBasePath)
    const response = await service.get()
  ...
  test('@GET - Absolute url', async () => {
    const service = new BeautifulApiClient(JSONPLACEHOLDER_URL).build(PostsApiService)
  ...
  test('@Field', async () => {
    const response = await service.field(PostsApiService.dto.userId, PostsApiService.dto.title, PostsApiService.dto.body)
  ...
  test('Methods that are not decorated should work', async () => {
    const withMethodsService = new BeautifulApiClient(testServer.url).build(WithMethodsService)
